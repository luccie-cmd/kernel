...
static void setAPICBase(uint64_t base) {
    io::wrmsr(IA32_APIC_BASE_MSR,
              (base & 0xFFFFF000) | (getAPICBase() & 0xFFF) | (1 << 11) | (1 << 10));
}

static void enableX2APIC() {
    io::wrmsr(IA32_APIC_BASE_MSR, getAPICBase() | (1 << 11) | (1 << 10));
}

static void lapicWrite(uint32_t offset, uint64_t value) {
    uint64_t base = getAPICBase();
    if (base & (1 << 10)) {
        io::wrmsr(0x800 + (offset / 16), value);
    } else {
        *(volatile uint32_t*)(getAPICBase() + offset) = (uint32_t)value;
    }
}

static uint64_t lapicRead(uint32_t offset) {
    uint64_t base = getAPICBase();
    if (base & (1 << 10)) {
        return io::rdmsr(0x800 + (offset / 16));
    } else {
        return *(volatile uint32_t*)(getAPICBase() + offset);
    }
}
...
void initLAPIC() {
    ...
    while (current_entry < entries_end) {
        ...
        case MADT_ENTRY_TYPE_PROCESSOR_LOCAL_X2APIC: {
            dbg::printm(MODULE, "X2APIC MADT Entry\n");
            std::abort();
        } break;
