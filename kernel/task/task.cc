...
void            initialize() {
    ...
    uint64_t codePhys = mmu::pmm::allocVirtual((codeSize + PAGE_SIZE - 1));
    uint64_t codeVirt = 0x123456;
    ...
    std::memcpy(reinterpret_cast<void*>(codeVirt), idleCodeBuffer, codeSize);
    currentProc->pid = pid;
    ...
    currentProc->registers->rip      = 0x123456;
...
void makeNewProcess(pid_t pid, uint8_t* codeBuffer, uint64_t codeSize, uint64_t entryPoint) {
    ...
    std::memcpy(reinterpret_cast<void*>(codeVirt), codeBuffer, codeSize);
...
void                nextProc() {
    ...
    dbg::printf("PID %llu has switchProc mapped to virtual 0x%llx -> 0x%llx\n", currentProc->pid,
                               (uint64_t)switchProc & (~0xFFF),
                               (mmu::vmm::getPhysicalAddr(mmu::vmm::getPML4(currentProc->pid),
                                                          (uint64_t)switchProc & (~0xFFF), false)));
...
// TODO: Syscall registers struct
struct __attribute__((packed)) SyscallRegs {
    uint64_t rbx, rip, rdx, rbp, rsi, rdi, r8, r9, r10, rflags, r12, r13, r14, r15, cr3, rax, rsp;
};
...
extern "C" void syscallHandler(SyscallRegs* regs) {
    ...
    currentProc->registers->orig_rsp = regs->rsp;
